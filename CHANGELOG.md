...
- Web PWA Docker image (`packages/web/Dockerfile`): the built bundle served by a small Express
  static server (SPA fallback, PWA-aware cache headers), with a GitHub Action publishing it to GHCR
  on each release, alongside the relay image.

### Fixed

- Remote control over a public relay: the daemon can now dial `wss://` (TLS) relays. It was built
  without a TLS feature, so it silently only spoke `ws://` and never joined the room on a hosted
  relay, leaving the web viewport stuck on "Disconnected".
